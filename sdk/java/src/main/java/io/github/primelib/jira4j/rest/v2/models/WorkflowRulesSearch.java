// WARNING: This file was generated by PrimeCodeGen. DO NOT EDIT.

package io.github.primelib.jira4j.rest.v2.models;

import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import com.fasterxml.jackson.annotation.JsonTypeName;
import java.util.List;
import java.util.Objects;
import java.util.function.Consumer;
import javax.annotation.processing.Generated;
import org.jetbrains.annotations.ApiStatus;

/**
 * WorkflowRulesSearch
 * <p>
 * Details of the workflow and its transition rules.
 *
 */
@JsonTypeName("WorkflowRulesSearch")
@JsonPropertyOrder({
    "expand",
    "ruleIds",
    "workflowEntityId"
})
@Generated(value = "io.github.primelib.primecodegen")
public class WorkflowRulesSearch {

    /**
     * Use expand to include additional information in the response. This parameter accepts `transition` which, for each rule, returns information about the transition the rule is assigned to.
     */
    @JsonProperty("expand")
    protected String expand;

    /**
     * The list of workflow rule IDs.
     */
    @JsonProperty("ruleIds")
    protected List<String> ruleIds;

    /**
     * The workflow ID.
     */
    @JsonProperty("workflowEntityId")
    protected String workflowEntityId;

    /**
     * Constructs a validated instance of {@link WorkflowRulesSearch}.
     *
     * @param spec the specification to process
     */
    public WorkflowRulesSearch(Consumer<WorkflowRulesSearch> spec) {
        super();
        spec.accept(this);
    }

    /**
     * Protected no-args constructor for use by serialization frameworks.
     */
    @ApiStatus.Internal
    protected WorkflowRulesSearch() {
    }

    /**
     * Constructs a validated instance of {@link WorkflowRulesSearch}.
     * <p>
     * NOTE: This constructor is not considered stable and may change if the model is updated. Consider using {@link #WorkflowRulesSearch(Consumer)} instead.
     * @param expand Use expand to include additional information in the response. This parameter accepts `transition` which, for each rule, returns information about the transition the rule is assigned to.
     * @param ruleIds The list of workflow rule IDs.
     * @param workflowEntityId The workflow ID.
     */
    @ApiStatus.Internal
    public WorkflowRulesSearch(String expand, List<String> ruleIds, String workflowEntityId) {
        this.expand = expand;
        this.ruleIds = ruleIds;
        this.workflowEntityId = workflowEntityId;
    }


    /**
     * Fluent getter for expand.
     * <p>
     * Use expand to include additional information in the response. This parameter accepts `transition` which, for each rule, returns information about the transition the rule is assigned to.
     *
     * @return expand
     */
    public String expand() {
        return this.expand;
    }

    /**
     * Fluent setter for expand.
     * <p>
     * Use expand to include additional information in the response. This parameter accepts `transition` which, for each rule, returns information about the transition the rule is assigned to.
     *
     * @param expand expand
     * @return this
     */
    public WorkflowRulesSearch expand(String expand) {
        this.expand = expand;
        return this;
    }

    /**
     * Gets the value of expand.
     * <p>
     * Use expand to include additional information in the response. This parameter accepts `transition` which, for each rule, returns information about the transition the rule is assigned to.
     *
     * @return expand
     */
    @JsonProperty("expand")
    public String getExpand() {
        return this.expand;
    }

    /**
     * Sets the value of expand.
     * <p>
     * Use expand to include additional information in the response. This parameter accepts `transition` which, for each rule, returns information about the transition the rule is assigned to.
     *
     * @param expand expand
     */
    public void setExpand(String expand) {
        this.expand = expand;
    }
    /**
     * Fluent getter for ruleIds.
     * <p>
     * The list of workflow rule IDs.
     *
     * @return ruleIds
     */
    public List<String> ruleIds() {
        return this.ruleIds;
    }

    /**
     * Fluent setter for ruleIds.
     * <p>
     * The list of workflow rule IDs.
     *
     * @param ruleIds ruleIds
     * @return this
     */
    public WorkflowRulesSearch ruleIds(List<String> ruleIds) {
        this.ruleIds = ruleIds;
        return this;
    }

    /**
     * Gets the value of ruleIds.
     * <p>
     * The list of workflow rule IDs.
     *
     * @return ruleIds
     */
    @JsonProperty("ruleIds")
    public List<String> getRuleIds() {
        return this.ruleIds;
    }

    /**
     * Sets the value of ruleIds.
     * <p>
     * The list of workflow rule IDs.
     *
     * @param ruleIds ruleIds
     */
    public void setRuleIds(List<String> ruleIds) {
        this.ruleIds = ruleIds;
    }
    /**
     * Fluent getter for workflowEntityId.
     * <p>
     * The workflow ID.
     *
     * @return workflowEntityId
     */
    public String workflowEntityId() {
        return this.workflowEntityId;
    }

    /**
     * Fluent setter for workflowEntityId.
     * <p>
     * The workflow ID.
     *
     * @param workflowEntityId workflowEntityId
     * @return this
     */
    public WorkflowRulesSearch workflowEntityId(String workflowEntityId) {
        this.workflowEntityId = workflowEntityId;
        return this;
    }

    /**
     * Gets the value of workflowEntityId.
     * <p>
     * The workflow ID.
     *
     * @return workflowEntityId
     */
    @JsonProperty("workflowEntityId")
    public String getWorkflowEntityId() {
        return this.workflowEntityId;
    }

    /**
     * Sets the value of workflowEntityId.
     * <p>
     * The workflow ID.
     *
     * @param workflowEntityId workflowEntityId
     */
    public void setWorkflowEntityId(String workflowEntityId) {
        this.workflowEntityId = workflowEntityId;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;
        WorkflowRulesSearch that = (WorkflowRulesSearch) o;
        return
            Objects.equals(this.expand, that.expand) &&
            Objects.equals(this.ruleIds, that.ruleIds) &&
            Objects.equals(this.workflowEntityId, that.workflowEntityId);
    }

    @Override
    public int hashCode() {
        return Objects.hash(
            this.expand, 
            this.ruleIds, 
            this.workflowEntityId
        );
    }

    @Override
    public String toString() {
        return "WorkflowRulesSearch{" +
            "expand=" + expand + ", " + 
            "ruleIds=" + ruleIds + ", " + 
            "workflowEntityId=" + workflowEntityId +
            "}";
    }
}
